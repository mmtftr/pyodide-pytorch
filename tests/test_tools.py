...
class ToolTests(unittest.TestCase):
    ...
    def test_playground_exposes_verified_transformer_benchmark(self) -> None:
        ...
        self.assertIn("torch.webgpu.synchronize()", app)
        self.assertIn("result = gpu_logits.cpu()", app)
        self.assertIn("actual_logits = gpu_logits.cpu()", app)
        worker = (ROOT / "site" / "worker.js").read_text(encoding="utf-8")
        self.assertIn('const ASSET_VERSION = "12"', app)
        self.assertIn('<script type="module" src="./app.js?v=12"></script>', index)
        self.assertIn("shell-v12", service_worker)
        self.assertIn("runtime-v7", service_worker)
        self.assertIn("./app.js?v=12", service_worker)
        self.assertIn("./worker.js?v=12", service_worker)
