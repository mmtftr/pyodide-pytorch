...
| Unsupported: promise-backed operations outside a JSPI entrypoint | `_local_scalar_dense`, native WebGPU-to-CPU `copy_`, synchronous `init()`/`synchronize()`/`.cpu()`/`.item()`/scalar truth entered through `runPython()` or a direct synchronous PyProxy call, and runtimes where `can_run_sync()` is false | Browser WebGPU exposes setup, queue completion, and mapping asynchronously. Use the explicit `init_async`, `synchronize_async`, and `to_cpu_async` fallbacks outside a JSPI-capable entrypoint. |
...
The next browser wheel also stages the remaining operators required by the
pinned BLOOM and T5 forward traces: restricted-Long tensor/scalar
add/subtract/multiply, `abs`, `minimum`, `gt.Scalar`, checked truncating
float-to-Long conversion, Long `where.self`, mixed float/integer multiply and
tensor power, plus fused float32 `baddbmm`. The hermetic Transformers gate now
requires all nine rows in its main matrix, with Gemma2 retained as the tenth
family in its dedicated two-layer mask/softcap gate. Together those pages are
the complete supported architecture profile; they are not a claim of coverage
for every PyTorch operator or arbitrary Hub model.
